...
export default function MerchantValetPaymentView({
  ...
  useEffect(() => {
    ...
    if (dispatchedBy) {
      ...
      const fallback = localStorage.getItem('dd_dispatch_wechat_qr') || localStorage.getItem('dd_wechat_qr') || localStorage.getItem('dd_dispatch_fee_qr_admin');
      if (fallback) {
        setDispatcherQr(fallback);
      }
    }
  }, [trip]);
  ...
  const qrImage = dispatcherQr || (trip as any)?.paymentQrCode || localStorage.getItem('dd_dispatch_wechat_qr') || localStorage.getItem('dd_wechat_qr');
